...
function tituloPagina(paginaAtual) {
  if (paginaAtual === "dashboard") return "Dashboard do inventário";
  if (paginaAtual === "setores") return "Setores e locais";
  ...
  if (paginaAtual === "backups") return "Backups";
  if (paginaAtual === "configuracoes") return "Configurações do sistema";
  return "Usuários dos computadores";
}

function subtituloPagina(paginaAtual) {
  if (paginaAtual === "dashboard") return "Resumo geral dos cadastros e movimentações do sistema";
  if (paginaAtual === "setores") return "Cadastro dos setores utilizados no inventário";
  ...
  if (paginaAtual === "backups") return "Acompanhamento dos backups locais do banco e anexos";
  if (paginaAtual === "configuracoes") return "Parâmetros gerais, segurança, backups, relatórios e ambiente";
...
function nomeModulo(paginaAtual) {
  ...
  if (paginaAtual === "setores") return "Setores";
  ...
  if (paginaAtual === "backups") return "Backups";
  if (paginaAtual === "configuracoes") return "Configurações";
  return "Computadores";
}
...
export default function Layout({
  ...
  function MenuConteudo({ mobile = false }) {
    const menuExpandido = menuAberto || mobile;
    ...
    return (
      <div className="flex h-full min-h-0 flex-col">
          ...
          <div className="flex min-w-0 items-center gap-3">
            ...
            {menuExpandido && (
              <div className="min-w-0">
                <p className="truncate text-sm font-semibold leading-tight">Inventário</p>
                <p className="truncate text-xs text-slate-400">Controle interno</p>
              </div>
            )}
          </div>
        ...
        <nav className="min-h-0 flex-1 overflow-y-auto px-3 py-4">
          <button type="button" onClick={() => navegar("dashboard")} className={itemClasse(paginaAtual === "dashboard")}>
            <span className={iconeClasse(paginaAtual === "dashboard")}><FiGrid size={18} /></span>
            {menuExpandido && <span>Dashboard</span>}
          </button>

          <button type="button" onClick={() => navegar("computadores")} className={`${itemClasse(paginaAtual === "computadores")} mt-2`}>
            <span className={iconeClasse(paginaAtual === "computadores")}><FiMonitor size={18} /></span>
            {menuExpandido && <span>Computadores</span>}
          </button>

          <button type="button" onClick={() => navegar("setores")} className={`${itemClasse(setoresAtivo)} mt-2`}>
            <span className={iconeClasse(setoresAtivo)}><FiPackage size={18} /></span>
            {menuExpandido && <span>Setores</span>}
          </button>
          ...
          <button type="button" onClick={() => navegar("manutencoes")} className={`${itemClasse(paginaAtual === "manutencoes")} mt-2`}>
            ...
            {menuExpandido && <span>Histórico</span>}
          </button>

          {permissoes?.podeGerarRelatorios && (
            <button type="button" onClick={() => navegar("relatorios")} className={`${itemClasse(paginaAtual === "relatorios")} mt-2`}>
              <span className={iconeClasse(paginaAtual === "relatorios")}><FiFileText size={18} /></span>
              {menuExpandido && <span>Relatórios</span>}
            </button>
          )}

          {podeVerAdministrador && (
            <div className="mt-2">
              <button type="button" onClick={() => alternarAdministrador(mobile)} className={itemClasse(administradorAtivo)}>
                <span className={iconeClasse(administradorAtivo)}><FiSettings size={18} /></span>
                {menuExpandido && (
                  <>
                    <span className="flex-1">Administrador</span>
                    <FiChevronDown size={16} className={`transition-transform ${adminAberto ? "rotate-180" : ""}`} />
                  </>
                )}
              </button>

              {adminAberto && menuExpandido && (
                <div className="mt-1 space-y-1 border-l border-white/10 pl-2">
                  {permissoes?.podeGerenciarUsuarios && (
                    <button type="button" onClick={() => navegar("usuarios")} className={subItemClasse(paginaAtual === "usuarios")}>
                      <FiUsers size={16} />
                      <span>Usuários</span>
                    </button>
                  )}
                  ...
                  {permissoes?.podeGerenciarBackups && (
                    <button type="button" onClick={() => navegar("backups")} className={subItemClasse(paginaAtual === "backups")}>
                      <FiArchive size={16} />
                      <span>Backups</span>
                    </button>
                  )}
                  ...
                  {permissoes?.podeGerenciarConfiguracoes && (
                    <button type="button" onClick={() => navegar("configuracoes")} className={subItemClasse(paginaAtual === "configuracoes")}>
                      <FiSettings size={16} />
          ...
          <button type="button" disabled className="mt-2 flex w-full items-center gap-3 px-4 py-3 text-left text-sm font-medium text-slate-500">
            <span className="flex h-7 w-7 items-center justify-center text-slate-500"><FiKey size={18} /></span>
            {menuExpandido && <span>Licenças</span>}
          </button>
        </nav>
        ...
        {menuExpandido && (
          <div className="shrink-0 border-t border-white/10 p-4 pb-[calc(1rem+env(safe-area-inset-bottom))]">
            <div className="mb-3">
              <p className="truncate text-sm font-bold text-white">{usuario?.nome || usuario?.username}</p>
              <p className="truncate text-xs text-slate-400">{usuario?.perfil_display}</p>
            </div>
        ...
        <header className="sticky top-0 z-20 border-b border-slate-200 bg-white/95 pt-[env(safe-area-inset-top)] backdrop-blur">
          <div className="flex min-h-16 items-center justify-between gap-3 px-4 py-3 sm:px-6 lg:h-16 lg:py-0">
            ...
            <div className="hidden shrink-0 text-right sm:block">
              <p className="text-xs font-semibold uppercase tracking-wide text-slate-400">{usuario?.perfil_display}</p>
              <p className="text-sm font-semibold text-slate-800">{nomeModulo(paginaAtual)}</p>
            </div>
          </div>
        </header>
